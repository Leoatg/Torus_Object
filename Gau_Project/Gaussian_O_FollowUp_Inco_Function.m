function     Gaussian_O_FollowUp_Inco_Function(Pmp,Pse,Psi,M)
...
% if Psi.fixed_int_r~=FS_fixed_int_r
%    fprintf('\nError: integration method does not fit.\n\n');
%    return;
% elseif FS_fixed_int_r==1 && (FS_int_r~=Psi.int_r)
%    fprintf('\nError: int_r does not fit.\n\n');
%    return;
% elseif FS_fixed_int_r==0 && (abs(FS_int_tolerant-Psi.int_tolerant)>1e-5)
%    fprintf('\nError: int_tolerant does not fit.\n\n');
%    return;
% end
...
if Pse.force_pulse_time<=TotalTime
    file_name_parameter=sprintf('Pmp0=%.2f_PmpPL=%.1f_PmpSmax=%d_PmpSmay=%d_Pse0=%.2f_PsePL=%.2f_PseSmax=%d_PseSmay=%d_N=%d_T=%d',...
...
if Psi.record_sxyz_int~=0
    save(strcat(currentFolder,'\',dataFolder,'\',file_name,'.mat'),'mod_psiLR_square','Ori_Pump_Parameter','record_psi_t','psiL_trim','psiR_trim','sx_int','sy_int','sz_int','Time','Energy','-mat','-v7.3');
else
    save(strcat(currentFolder,'\',dataFolder,'\',file_name,'.mat'),'mod_psiLR_square','Ori_Pump_Parameter','record_psi_t','psiL_trim','psiR_trim','Time','Energy','-mat','-v7.3');
end
