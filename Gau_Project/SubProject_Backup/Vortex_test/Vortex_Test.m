...
figure('renderer','zbuffer','position',[ 704   371   767   689],'paperpositionmode','auto');
x_b=25;
%PhaseSlop+Phase_V
mesh(x,y,PhaseSlope+Phase_V);
axis square;
...
% 
% x_b=25;
% phiR=wrapToPi(PhaseSlop+Phase_V);
% subplot(1,3,1);
% mesh(x,y,phiR);
% axis square;
...
% 
% subplot(1,3,2);
% mesh(x,y,unwrap(unwrap(phiR,[],1),[],2));
% axis square;
% set(gca,'fontsize',Font_size,'xlim',[-x_b x_b],'ylim',[-x_b,x_b]);
% %colorbar;
% xlabel('x');
% ylabel('y');
% view(-79,8);
% title('phase unwrapped by Matlab');
% 
% subplot(1,3,3);
% mesh(x,y,unwrap(unwrap(phiR,[],1),[],2));
% axis square;
...
% colorbar;
% view(0,90);
% title('phase unwrapped by Matlab, viewed from top');
% 
% 
% print(gcf,'-zbuffer','-dpng','-r250','Artifical_vortex_Multiple.png');
