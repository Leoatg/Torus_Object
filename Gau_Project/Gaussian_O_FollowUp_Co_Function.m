function     Gaussian_O_FollowUp_Co_Function(Pmp,Pse,Psi,M)
...
file_name=strcat('GauPsi_',M.Simulation_version,'_CoPulse_',file_name_parameter);

if Pse.force_pulse_time<=TotalTime
    file_name_parameter=sprintf('Pmp0=%.2f_PmpPL=%.1f_PmpSmax=%d_PmpSmay=%d_Pse0=%.2f_PsePL=%.1f_PseSmax=%d_PseSmay=%d_N=%d_T=%d',...
...
subplot(3,3,7)
plot(Psi.Time,Psi.mod_psiLR_square);
axis square;
title('|\psi_L|^2+|\psi_R|^2 int');

subplot(3,3,8)
plot(Psi.Time,Psi.Energy./Psi.mod_psiLR_square);
axis square;
title('E/|psiLR|');
    
if Psi.record_sxyz_int~=0
    subplot(3,3,9)
    plot(Psi.Time,Psi.sz_int);
    axis square;
    title('sz int');
end
