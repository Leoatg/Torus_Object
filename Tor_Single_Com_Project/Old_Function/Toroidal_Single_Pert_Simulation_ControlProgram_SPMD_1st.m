...
Pert_Strength=0.1; %will multiply to the steady state mode
Pert_Fre=-10; %Perturbation density spacial variation frequency  m+Fre, m-Fre
...
%SPMD preparation
m0_series=0:3;
N_m0=length(m0_series);

%Cluster preparation
%My_Cluster=parallel.importProfile('PBSProProfile1.settings');
%My_Cluster=parallel.importProfile('local_test1.settings');
if matlabpool('size')==0
   %matlabpool(My_Cluster,N_m0);
   matlabpool;
end

spmd (N_m0)
Psi.Ini_m=m0_series(labindex);
Toroidal_O_Single_Pert_Simulation_Inco_Function(Pmp,Pse,Psi,M);
fprintf('Task %d had %d cores available',labindex,feature('numcores'));
end
