function     Toroidal_O_Single_Pert_Simulation_Inco_Function(Pmp,Pse,Psi,M)
...
file_name_parameter=sprintf('GmaRr=%.1f_m0=%d_Pmp0=%.1f_l=%d_p=%d_w0=%d_PerS=%g_PerF=%d_PerT=%d_XYmax=%d_N=%d_T=%d',Psi.GammaR/Psi.GammaC,Psi.Ini_m,Pmp.Pbar0,Pmp.l,Pmp.p,Pmp.w0,M.Pert_Strength,M.Pert_Fre,M.Pert_time,Pmp.XYmax,N,floor(TotalTime));
...
for step_i=1:floor(TotalTime/dt)
    ...
    if M.Pert_time>0 && M.Pert_Strength~=0 && time>M.Pert_time  %nagetive value means perturbation added
        ...
        Pert=psi_mode.*(rho_1*exp(1i*(phi0+k_per)*theta)+rho_2*exp(1i*(phi0-k_per)*theta))*M.Pert_Strength;
...
if Psi.record_psi~=0 %time series of psi is saved
    ...
    n_trim=Psi.n_trim;
    save(file_path,'Finial_State','N2_P_eff','N1_P_eff','P_eff','FS_fixed_int_r','Ori_Pump_Parameter','record_psi_t','psi_trim','n_trim','Time','Energy','Psi_int','Lz','-mat','-v7.3');
else
    ...
    elseif Psi.record_finial_state==1 %save Pump and Energy
        ...
        save(file_path,'N2_P_eff','N1_P_eff','P_eff','FS_fixed_int_r','Ori_Pump_Parameter','Time','Energy','Psi_int','Lz','-mat','-v7.3');
    elseif Psi.record_finial_state==2 %save the finial state
        fprintf('Storing Data\n ');
        Finial_State=complex(0,zeros(N,N,2));
        Finial_State(:,:,1)=psi;
        Finial_State(:,:,2)=n;
        save(file_path,'N2_P_eff','N1_P_eff','P_eff','FS_fixed_int_r','Ori_Pump_Parameter','Time','Energy','Psi_int','Finial_State','Lz','-mat','-v7.3');
    end
end
