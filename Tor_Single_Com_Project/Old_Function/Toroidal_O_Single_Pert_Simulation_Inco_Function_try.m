function     Toroidal_O_Single_Pert_Simulation_Inco_Function_try(Pmp,Pse,Psi,M)
...
XYmax=Pmp.XYmax;
global OS;
global NoFigureWindow_Flag;
global File_name_append;
global Tesla_Cool_down_period;
global Noise_Strength;
global k_min k_space k_max Multiple_k
...
% record psi point
global psi_point
N_pp=length(psi_point);
record_psi_point=complex(zeros(N_pp,length(Psi.Time)));

%% Saved data file name
file_name_parameter=sprintf('GmR=%.1f_m0=%d_Pmp0=%.1f_l=%d_p=%d_w0=%d_PerS=%g_PerF=%.1f_PerT=%d_XYmax=%d_N=%d_T=%d',Psi.GammaR/Psi.GammaC,Psi.Ini_m,Pmp.Pbar0,Pmp.l,Pmp.p,Pmp.w0,M.Pert_Strength,M.Pert_Fre,M.Pert_time,Pmp.XYmax,N,floor(TotalTime));
Mov_file_name_psi=strcat(file_name_parameter);
file_name=[file_name_parameter,File_name_append];
...
if M.GPU_calculation~=0
    ...
    if M.GPU_calculation>=2 %full GPU calculation
        ...
        record_psi_point_gpu=gpuArray(record_psi_point);
...
for step_i=1:floor(TotalTime/dt)
    ...
    %cool down Tesla
    if Tesla_Cool_down_period~=0
        if M.GPU_calculation==3 && ((TotalTime-time)>1)
            if N>=2048
                if mod(time,1)==0 && (time>1)
    ...
    if M.Pert_time>0 && M.Pert_Strength~=0 && time>M.Pert_time  %nagetive value means perturbation added
        ...
        Pert_psi=psi_mode.*Pert_psi*M.Pert_Strength;
        
        Pert_n=n.*Pert_n*M.Pert_Strength;
    ...
    if mod(step_i,Psi.E_step_interval)==0
        ...
        if M.GPU_calculation>=2 %record Energy within the GPU
            ...
            for it_p=1:N_pp
                ...
                x_r=cos(psi_point(it_p))*R0;
                y_r=sin(psi_point(it_p))*R0;
...
if M.GPU_calculation>=2
    ...
    record_psi_point=gather(record_psi_point_gpu);
...
%Unornalized Energy
subplot(2,2,2)
h_plot=plot(Time,Energy_unmal,'LineWidth',3);
...
title(sprintf('L_z (normalized)  m_0=%d;  L_z final=%.1f',Psi.Ini_m,real(Lz(length(Lz)))));
    ...
    file_path_temp=[currentFolder,'\',dataFolder,'\'];
    
    if numlabs~=1 || NoFigureWindow_Flag~=0 %SPMD or NoFigureWindows
        print(fig_E,'-dpng',[file_path_temp,file_name,'.png']);
    else
        print(fig_E,'-zbuffer','-dpng','-r150',[file_path_temp,file_name,'.png']);
    end
else %Linux
    ...
    plot(Time,real(Energy_unmal),'LineWidth',3);
    axis square;
    set(gca,'fontsize',Psi.font_size);
    set(gca,'fontname',Font_name);
    xlabel('t');
...
if Psi.record_psi~=0 %time series of psi is saved
    ...
    Finial_State(:,:,1)=psi;
    Finial_State(:,:,2)=n;
    save(file_path,'Finial_State','N2_P_eff','N1_P_eff','Pth_cutoff','psi_point','record_psi_point','E_t_interval','P_eff','FS_fixed_int_r','Ori_Pump_Parameter','record_psi_t','psi_trim','n_trim','Time','Energy','Psi_int','Lz','-mat','-v7.3');
else
    ...
    elseif Psi.record_finial_state==1 %save Pump and Energy
        ...
        save(file_path,'N2_P_eff','N1_P_eff','Pth_cutoff','psi_point','record_psi_point','E_t_interval','P_eff','FS_fixed_int_r','Ori_Pump_Parameter','Time','Energy','Psi_int','Lz','-mat','-v7.3');
    elseif Psi.record_finial_state==2 %save the finial state
        fprintf('Storing Data\n');
        Finial_State=complex(0,zeros(N,N,2));
        Finial_State(:,:,1)=psi;
        Finial_State(:,:,2)=n;
        save(file_path,'N2_P_eff','N1_P_eff','Pth_cutoff','psi_point','record_psi_point','E_t_interval','P_eff','FS_fixed_int_r','Ori_Pump_Parameter','Time','Energy','Psi_int','Finial_State','Lz','-mat','-v7.3');
    end
end
