...
%                 end
    ...
    if M.Pert_time>0 && M.Pert_Strength~=0 && time>M.Pert_time  %nagetive value means perturbation added
        ...
        Pert_n=n.*(w_0*exp(1i*k_per*theta)+w_0*exp(-1i*k_per*theta)+Noise)*M.Pert_Strength;
...
saveas(fig_E,strcat(currentFolder,'\',dataFolder,'\',file_name,'.png'),'png');
    ...
    print(fig_E,'-painter','-dpng','-r300',strcat(currentFolder,'\',dataFolder,'\',file_name,'.png'));
...
if Psi.record_psi~=0 %time series of psi is saved
    ...
    n_trim=Psi.n_trim;
    save(file_path,'Finial_State','N2_P_eff','N1_P_eff','Pth_cutoff','P_eff','E_t_interval','FS_fixed_int_r','Ori_Pump_Parameter','record_psi_t','psi_trim','n_trim','Time','Energy','Psi_int','Lz','-mat','-v7.3');
else
    ...
    elseif Psi.record_finial_state==1 %save Pump and Energy
        ...
        save(file_path,'N2_P_eff','N1_P_eff','Pth_cutoff','P_eff','E_t_interval','FS_fixed_int_r','Ori_Pump_Parameter','Time','Energy','Psi_int','Lz','-mat','-v7.3');
    elseif Psi.record_finial_state==2 %save the finial state
        fprintf('Storing Data\n ');
        Finial_State=complex(0,zeros(N,N,2));
        Finial_State(:,:,1)=psi;
        Finial_State(:,:,2)=n;
        save(file_path,'N2_P_eff','N1_P_eff','Pth_cutoff','P_eff','E_t_interval','FS_fixed_int_r','Ori_Pump_Parameter','Time','Energy','Psi_int','Finial_State','Lz','-mat','-v7.3');
    end
end
