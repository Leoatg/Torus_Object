...
global File_name_append;File_name_append='';

component_number=1;

%% Perturbation Parameters
Pert_Strength=0.05; %will multiply to the steady state mode
Pert_Fre=75; %Perturbation density spacial variation frequency  m+Fre, m-Fre
Pert_m0=0; %angular momentum of perturbation
Pert_time=45;
global Noise_Strength;Noise_Strength=0;
...
GPU_calculation=3;%0:off, 1:Half GPU, 2:full GPU 3:Tesla 4:Quadro %present in the Mov_record class

global Tesla_Cool_down_period;  %allow Tesla cool down period
Tesla_Cool_down_period=1; %0:off, 1:on

Send_mail=0;
Pop_up_notice=1;

%% Record Finial State
record_finial_state=2; %0:no recording; 1: record Pump and energy; 2:record finial state
...
global NoFigureWindow_Flag; %1:noFigureWindows
NoFigureWindow_Flag=~feature('ShowFigureWindows');
if NoFigureWindow_Flag~=0
    Pop_up_notice=0;
end
...
%SPMD preparation
Parameter_series=5:5:50;
N_p=length(Parameter_series);

%Cluster preparation
if matlabpool('size')~=0
    fprintf(sprintf('\nExinting pool size=%d\n\n',matlabpool('size')));
    matlabpool close force local;
else
    fprintf('\nNo existing pool was found.\n\n');
end
if N_p<12
    matlabpool('local',N_p);
else
    matlabpool('local',12);
end
...
TempFolder_Ori=getenv('TMP');
SimFolder=getenv('SIMPATH');

HomeFolder='/home/124/gyl124';
cd(HomeFolder);

spmd (Pool_size)
    for it_n=labindex:Pool_size:N_p
        
        M.Pert_Fre=Parameter_series(it_n);
        
        fprintf(sprintf('it_n=%d\n',it_n));
        
        try
            Toroidal_O_Single_Pert_Simulation_Inco_Function(Pmp,Pse,Psi,M);
        catch err
            fprintf(['Error occured:',err.identifier,'\n']);
            fprintf([err.message,'\n'])
            fprintf('program stopped.\n');
            break;
        end
