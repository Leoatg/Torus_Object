...
% stability analysis, dispertion analysis; Normalized Fluctuations
...
for it_P_series=1:length(Parameter_series(:,3))
    ...
    for it=1:N_k
        ...
        L=[((m+k(it))^2-m^2)/(2*R0^2)+ua*Phi0^2, ua*Phi0^2, (g+0.5i*R1)*n0;
            -ua*Phi0^2, -(((m-k(it))^2-m^2)/(2*R0^2)+ua*Phi0^2), -(g-0.5i*R1)*Phi0;
            -1i*R1*Phi0^2, -1i*R1*Phi0^2, -1i*(GammaR+R1*Phi0^2)];
    ...
    if print_fig~=0
        ...
        print(fig_Re,'-painter','-depsc','-r250',sprintf('Dispersion_Curve_Re_m=%d_GammaR=%.1f_P1=%.1f_Nor_fluctu.eps',m,GammaR,P1));
        print(fig_Im,'-painter','-depsc','-r250',sprintf('Dispersion_Curve_Im_m=%d_GammaR=%.1f_P1=%.1f_Nor_fluctu.eps',m,GammaR,P1));
