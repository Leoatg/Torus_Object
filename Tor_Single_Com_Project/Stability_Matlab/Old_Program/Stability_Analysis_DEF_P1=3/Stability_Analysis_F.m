...
if Record_Mov_m~=0
    writerObj = VideoWriter('Dispersion_mov_F.avi');
...
axis square;
set(axis_re,'ylim',[-3,3],'fontsize',Fontsize);
xlabel('k');
ylabel('Re \omega');
title(axis_re,sprintf('m=%d',m));
hold(axis_re,'all');
...
axis square;
set(axis_im,'ylim',[-3.5,0.5],'fontsize',Fontsize);
xlabel('k');
ylabel('Imag \omega');
hold all
...
if print_fig~=0
    print(gcf,'-painter','-dpng','-r250',sprintf('Dispersion curve_F_m=%d.png',m));
    close(fig_disp);
end
