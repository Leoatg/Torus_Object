...
for m_it=1:N_m
    ...
    for it=1:N
        ...
        E2_1(it)=real(D(1,1))*(abs(V(1,1))^2-abs(V(2,1))^2)*2*pi*exp(2*imag(D(1,1)*time_t))+(real(V(1,1))+real(V(2,1)))*V(3,1)*pi*(g+0.5i*R1);
        E2_2(it)=real(D(2,2))*(abs(V(1,2))^2-abs(V(2,2))^2)*2*pi*exp(2*imag(D(2,2)*time_t))+(real(V(1,2))+real(V(2,2)))*V(3,2)*pi*(g+0.5i*R1);
        E2_3(it)=real(D(3,3))*(abs(V(1,3))^2-abs(V(2,3))^2)*2*pi*exp(2*imag(D(3,3)*time_t))+(real(V(1,3))+real(V(2,3)))*V(3,3)*pi*(g+0.5i*R1);
        ...
        E2_de=sortrows(E2_de,-1);%sortrow for E2
    ...
    if show_fig~=0
        ...
        hold off
        title(['Re E2',sprintf('     t=%d    m=%d',time_t,m)])
        ...
        hold off
        title('Im E2');
        h_l=legend('$E^{(2)}_C 1$','$E^{(2)} 2$','$E^{(2)} 3$');
        set(h_l,'Interpreter','latex');
...
if print_fig~=0 && Record_Mov_m==0
    print(gcf,'-painters','-dpng','-r250',sprintf('E2_m=%d_GammaR=%.1f_P1=%.1f_w0=%d_t=%d.png',m,GammaR,P1,w0,time_t));
    close(gcf);
end
