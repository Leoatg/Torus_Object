...
N=length(k);
omega_1_pra=m*k+sqrt(k.^2.*(c+k.^2/4));
omega_2_pra=m*k-sqrt(k.^2.*(c+k.^2/4));

omega_1_my=(2*k*m+sqrt(k.^4+k.^2*4*ua*R0^2*Phi0^2))/(2*R0^2);
omega_2_my=(2*k*m-sqrt(k.^4+k.^2*4*ua*R0^2*Phi0^2))/(2*R0^2);

fig_conv=figure('renderer','painter','position',[100 100 1600 800],'paperpositionmode','auto');

plot(k,omega_1_my,'linewidth',2);
axis square;
set(gca,'fontsize',Fontsize);
xlabel('k');
ylabel('\omega');
title(gca,sprintf('m=%d, Gamma_R=%.1f, P bar=%.1f, R0=%.1f, c=%.2f',m,GammaR,P1,R0,c));
hold all;
plot(k,omega_2_my,'linewidth',2);
plot(gca,k,zeros(1,N),'linestyle','-.','linewidth',2);
%plot(gca,k,zeros(1,N).','linestyle','-.','linewidth',2);
hold off;
...
% xlabel('k');
% ylabel('Re \omega');
% title(axis_re,sprintf('m=%d, Gamma_R=%.1f, P bar=%.1f, R0=%.1f',m,GammaR,P1,R0));
% hold(axis_re,'all');
