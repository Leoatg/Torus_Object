function     Toroidal_O_GaussianPulse_FollowUp_Inco_Function(Pmp,Pse,Psi,M)
...
if Pmp.N==2048
    dataFolder=strcat('TorPsi_',M.Simulation_version,'_N2048_IncoPulse_',folder_name_parameter);
else
...
if ~Pmp.Check_Parameter(Ori_Pump_Parameter);
   fprintf('\nError: Pump parameters do not fit.\n\n');
   return;
end
...
if Pse.force_pulse_time<=TotalTime
    file_name_parameter=sprintf('IR=%.1f,J=%.1f,Pmp0=%.2f_l=%d_p=%d_w0=%d_Pse0=%.2f_Sigmax=%d_Sigmay=%d_x0=%d_y0=%d_T=%d',IR,J,Pmp.Pbar0,Pmp.l,Pmp.p,Pmp.w0,Pse.Pbar0,Pse.Sigma_x,Pse.Sigma_y,Pse.x0,Pse.y0,floor(TotalTime));
    Mov_file_name_psi=strcat('TorPsi_',M.Simulation_version,'_IncoPulse_',file_name_parameter);
    Mov_file_name_sxyz=strcat('Torsxyz_',M.Simulation_version,'_IncoPulse_',file_name_parameter);
    file_name=strcat('TorPsi_',M.Simulation_version,'_IncoPulse_',file_name_parameter);
end
...
for step_i=step_0:floor(TotalTime/dt)
    ...
    elseif Pulse_add_flag==1 %stage 2
        Pulse_time=time-Pse.force_pulse_time;
        adiabatic_term=0.5*(1+tanh(Pse.ada_coe*(Pulse_time-Pse.ada_shift)));
        PumpL=abs(sqrt(Pmp.PL)*Pmp.u+sqrt(Pse.PL)*sqrt(adiabatic_term)*Pse.u).^2;
        PumpR=abs(sqrt(1-Pmp.PL)*Pmp.u+sqrt(1-Pse.PL)*sqrt(adiabatic_term)*Pse.u).^2;
        if M.GPU_calculation==2
            PumpL_gpu=gpuArray(PumpL);
            PumpR_gpu=gpuArray(PumpR);
        end
