function     Toroidal_O_TwoCom_FollowUp_Inco_Function(Pmp,Pse,Psi,M)

%Follow program with Incoherent pulse (Object oriented)
%save Time Energy
%record mov
%record trimmed psi
fprintf('\nTwo Component FollowUp Inco program.\n')
tic;
...
load(strcat(currentFolder,'\',dataFolder,'\','Tor_Standard_Steady_State_',M.Simulation_version,'_m0_',num2str(Psi.Ini_m),'.mat'));
...
file_name_parameter=sprintf('m0=%d_Pmp0=%.2f_l=%d_p=%d_w0=%d_N=%d_T=%d',Psi.Ini_m,Pmp.Pbar0,Pmp.l,Pmp.p,Pmp.w0,N,floor(TotalTime));
...
if Pse.force_pulse_time<=TotalTime
    file_name_parameter=sprintf('m0=%d_Pmp0=%.2f_l=%d_p=%d_w0=%d_Pse0=%.2f_LP=%d_pP=%d_w0P=%d_OmegaP=%.3f_N=%d_T=%d',Psi.Ini_m,Pmp.Pbar0,Pmp.l,Pmp.p,Pmp.w0,Pse.Pbar0,Pse.l,Pse.p,Pse.w0,Pse.Omega,N,floor(TotalTime));
    Mov_file_name_psi=strcat('TorPsi_',M.Simulation_version,'_IncoPulse_',file_name_parameter);
    Mov_file_name_sxyz=strcat('Torsxyz_',M.Simulation_version,'_IncoPulse_',file_name_parameter);
    file_name=strcat('TorPsi_',M.Simulation_version,'_IncoPulse_',file_name_parameter);
end
...
if floor(step_0*dt)>Pse.force_pulse_time
   fprintf('\nError: Force pulse time is smaller than the steady state time\n\n');
   return;
end
