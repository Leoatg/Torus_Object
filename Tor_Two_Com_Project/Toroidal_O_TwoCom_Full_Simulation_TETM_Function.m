function     Toroidal_O_TwoCom_Full_Simulation_TETM_Function(Pmp,Pse,Psi,M)
...
IR=Psi.IR;
Dk=Psi.Dk;%TE-TM splitting coupling
hspace=Psi.hspace;
...
file_name_parameter=sprintf('m0=%d_Pmp0=%.2f_PmpPL=%.2f_l=%d_p=%d_w0=%d_XYmax=%d_N=%d_T=%d',Psi.Ini_m,Pmp.Pbar0,Pmp.PL,Pmp.l,Pmp.p,Pmp.w0,Pmp.XYmax,N,floor(TotalTime));
...
if Pse.force_pulse_time<=TotalTime
    file_name_parameter=sprintf('m0=%d_Pmp0=%.2f_PmpPL=%.1f_l=%d_p=%d_w0=%d_Pse0=%.2f_PsePL=%.1f_LP=%d_pP=%d_w0P=%d_OmegaP=%.4f_N=%d_T=%d',Psi.Ini_m,Pmp.Pbar0,Pmp.PL,Pmp.l,Pmp.p,Pmp.w0,Pse.Pbar0,Pse.PL,Pse.l,Pse.p,Pse.w0,Pse.Omega,N,floor(TotalTime));
    Mov_file_name_psi=strcat('TorPsi_',M.Simulation_version,'_IncoPulse_',file_name_parameter);
    Mov_file_name_sxyz=strcat('Torsxyz_',M.Simulation_version,'_IncoPulse_',file_name_parameter);
    file_name=strcat('Psi_',M.Simulation_version,'_IcoPse_',file_name_parameter);
end

currentFolder = pwd;
folder_name_parameter=sprintf('IR=%.1f_J=%.1f',IR,J);
dataFolder=strcat('TorPsi_',M.Simulation_version,'_IncoPulse_',folder_name_parameter);

if exist(dataFolder,'dir')==0
    mkdir(dataFolder);
end

M.GPU_calculation=0;%no GPU calculation
...
for step_i=1:floor(TotalTime/dt)
    ...
    [psiL_x,psiL_y]=gradient(psiL1,hspace);
    [psiR_x,psiR_y]=gradient(psiR1,hspace);
    ...
    psiL_temp=psiL1;
    psiL1=psiL1.*exp((ua*abs(psiL1).^2+ub*abs(psiR1).^2+gR*(nL)+0.5i*(R*nL-GammaC))*(-1i)*dt)-1i*J*psiR1*dt+(-1i)*dt*Dk*(-psiR_xx-2i*psiR_xy+psiR_yy);
    psiR1=psiR1.*exp((ua*abs(psiR1).^2+ub*abs(psiL_temp).^2+gR*(nR)+0.5i*(R*nR-GammaC))*(-1i)*dt)-1i*J*psiL_temp*dt+(-1i)*dt*Dk*(-psiL_xx+2i*psiL_xy+psiL_yy);
...
if Psi.record_finial_state==0 && Psi.record_psi~=0
    ...
    save(strcat(currentFolder,'\',dataFolder,'\',file_name,'.mat'),'FS_fixed_int_r','Ori_Pump_Parameter','record_psi_t','psiL_trim','psiR_trim','Time','Energy','-mat','-v7.3');
else
    ...
    Finial_State(:,:,3)=nL;
    Finial_State(:,:,4)=nR;
    save(strcat(currentFolder,'\',dataFolder,'\','FinalState_',file_name,'.mat'),'FS_fixed_int_r','Ori_Pump_Parameter','Time','Energy','Finial_State','-mat','-v7.3');
end
