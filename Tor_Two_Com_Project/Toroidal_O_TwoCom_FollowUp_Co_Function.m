function     Toroidal_O_TwoCom_FollowUp_Co_Function(Pmp,Pse,Psi,M)
...
if Pmp.N==2048
    dataFolder=strcat('TorPsi_',M.Simulation_version,'_N2048_CoPulse_',folder_name_parameter);
else
    dataFolder=strcat('TorPsi_',M.Simulation_version,'_CoPulse_',folder_name_parameter);
end

load(strcat(currentFolder,'\',dataFolder,'\','Tor_Standard_Steady_State_',M.Simulation_version,'.mat'));
...
r_damping=XYmax-Ori_Pump_Parameter.damping_shift;%radius for top-hat damping
a_damping=Ori_Pump_Parameter.a_damping; %slope for the damping mesa
...
file_name_parameter=sprintf('IR=%.1f,J=%.1f,Pmp0=%.2f_l=%d_p=%d_w0=%d_T=%d',IR,J,Pmp.Pbar0,Pmp.l,Pmp.p,Pmp.w0,floor(TotalTime));
Mov_file_name_psi=strcat('TorPsi_',M.Simulation_version,'_CoPulse_',file_name_parameter);
Mov_file_name_sxyz=strcat('Torsxyz_',M.Simulation_version,'_CoPulse_',file_name_parameter);
file_name=strcat('TorPsi_',M.Simulation_version,'_CoPulse_',file_name_parameter);

if Pse.force_pulse_time<=TotalTime
    file_name_parameter=sprintf('IR=%.1f,J=%.1f,Pmp0=%.2f_l=%d_p=%d_w0=%d_Pse0=%.3f_LP=%d_pP=%d_w0P=%d_OmegaP=%.3f_T=%d',IR,J,Pmp.Pbar0,Pmp.l,Pmp.p,Pmp.w0,Pse.Pbar0,Pse.l,Pse.p,Pse.w0,Pse.Omega,floor(TotalTime));
    Mov_file_name_psi=strcat('TorPsi_',M.Simulation_version,'_CoPulse_',file_name_parameter);
    Mov_file_name_sxyz=strcat('Torsxyz_',M.Simulation_version,'_CoPulse_',file_name_parameter);
    file_name=strcat('TorPsi_',M.Simulation_version,'_CoPulse_',file_name_parameter);
end
...
for step_i=step_0:floor(TotalTime/dt)
    ...
    elseif Pulse_add_flag==1 %stage 2
        ...
        %Pmp.Update_u();
        Pse.Update_u();
        %PumpL=abs(Pmp.PL*Pmp.u+Pse.PL*Pse.u);
        %PumpR=abs((1-Pmp.PL)*Pmp.u+(1-Pse.PL)*Pse.u);
%         if M.GPU_calculation==2
%             PumpL_gpu=gpuArray(PumpL);
%             PumpR_gpu=gpuArray(PumpR);
%         end
...
%             Pmp.time=time;
%             %Pmp.Update_u();
%             PumpL=Pmp.PL*abs(Pmp.u);
%             PumpR=(1-Pmp.PL)*abs(Pmp.u);
%             if M.GPU_calculation==2
%                 PumpL_gpu=gpuArray(PumpL);
%                 PumpR_gpu=gpuArray(PumpR);
%             end
...
subplot(2,3,6)
plot(double(Psi.Time),Psi.Energy);
axis square;
title('Energy');

%saveas(fig_E,strcat(currentFolder,'\',dataFolder,'\',file_name,'.png'),'png');
print(fig_E,'-zbuffer','-dpng','-r200',strcat(currentFolder,'\',dataFolder,'\',file_name,'.png'));
close(fig_E);
